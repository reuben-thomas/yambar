...
struct watcher {
    ...
    // This bus should is the tray's bus.
    //
    sd_bus *bus;
    hosts_t hosts;
    items_t items;
...
get_property_callback(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (sd_bus_message_is_method_error(msg, NULL)) {
        ...
        if ((!strcmp(prop, "IconThemePath")) && (!strcmp(err->name, SD_BUS_ERROR_UNKNOWN_PROPERTY))) {
            ...
            LOG_WARN("[SNI]: %s %s: %s", sni->watcher_id, prop, err->message);
        }
...
cleanup:
    ...
    {
        if (it->item == d) {
            tll_remove_and_free(sni->slots, it, destroy_slot);
            break;
        }
    };
    return ret;
...
sni_get_property_async(struct sni *sni, const char *prop, const char *type, void *dest)
{
    ...
    data->dest = dest;
    int ret
        = sd_bus_call_method_async(sni->m->bus, &data->slot, sni->service, sni->path, "org.freedesktop.DBus.Properties",
                                   "Get", get_property_callback, data, "ss", sni->interface, prop);

    if (ret >= 0) {
        tll_push_front(sni->slots, data);
    } else {
        LOG_ERR("%s %s: %s", sni->watcher_id, prop, strerror(-ret));
        free(data);
    }
}
...
{
    struct sni *sni = data;
    LOG_DBG("[Signal Handle] %s: NewOverlayIcon", sni->watcher_id);
...
handle_new_status(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    struct sni *sni = data;
    LOG_DBG("[Signal Handler] %s: NewStatus", sni->watcher_id);
    ...
    if (ret == 1) {
        ...
        if (r < 0) {
            ...
            LOG_DBG("[Signal Handler] %s: NewStatus = '%s'", sni->watcher_id, status);
            sni->m->mod->bar->refresh(sni->m->mod->bar);
        }
    } else {
...
{
    struct sni *sni = data;
    LOG_DBG("[Signal Handler] %s: NewTitle", sni->watcher_id);
...
handle_lost_service(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (!*new_owner) {
        ...
        {
            const char *item_name = it->item;
            int cmp_res = using_standard_protocol(watcher) ? strcmp(item_name, service)
                                                           : strncmp(item_name, service, strlen(service));
            if (cmp_res == 0) {
                LOG_DBG("[Signal Handler] %s: unregistering Status Notifier Item = '%s'", watcher->interface,
                        item_name);
                sd_bus_emit_signal(watcher->bus, watcher_path, watcher->interface, "StatusNotifierItemUnregistered",
                                   "s", item_name);
                tll_remove_and_free(watcher->items, it, free);
                sd_bus_emit_properties_changed(watcher->bus, watcher_path, watcher->interface,
        ...
        tll_foreach(watcher->hosts, it)
        {
            if (strcmp(it->item, service) == 0) {
                LOG_DBG("[Signal Handler] %s: unregistering Status Notifier Host = '%s'", watcher->interface, service);
                tll_remove_and_free(watcher->hosts, it, free);
                if (tll_length(watcher->hosts) == 0) {
                    sd_bus_emit_properties_changed(watcher->bus, watcher_path, watcher->interface,
...
register_sni(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (found) {
        LOG_DBG("[Callback] %s: Status Notifier Item already registered = '%s'", watcher->interface, id);
        free(id);
    } else {
        LOG_DBG("[Callback] %s: registering Status Notifier Item = '%s'", watcher->interface, id);
        ...
        sd_bus_emit_properties_changed(watcher->bus, watcher_path, watcher->interface, "RegisteredStatusNotifierItems",
                                       NULL);
    }
...
register_host(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (found) {
        LOG_DBG("[Callback] %s: Status Notifier Host already registered = '%s'", watcher->interface, service);
    } else {
        LOG_DBG("[Callback] %s: registering watcher to host = '%s'", watcher->interface, service);
        tll_push_back(watcher->hosts, strdup(service));
        if (tll_length(watcher->hosts) == 1) {
            sd_bus_emit_properties_changed(watcher->bus, watcher_path, watcher->interface,
                                           "IsStatusNotifierHostRegistered", NULL);
        }
        sd_bus_emit_signal(watcher->bus, watcher_path, watcher->interface, "StatusNotifierHostRegistered", "");
    }
...
{
    struct watcher *watcher = data;
    LOG_DBG("[Callback] %s: get registered snis", watcher->interface);
    int r = sd_bus_message_open_container(reply, 'a', "s");
    if (r < 0)
        return r;
    tll_foreach(watcher->items, it)
    {
        LOG_DBG("[Status Notifier Item] %s has %s", watcher->interface, it->item);
...
{
    struct watcher *watcher = data;
    LOG_DBG("[Callback] %s: is host registered = '%d'", watcher->interface, tll_length(watcher->hosts) > 0);
...
{
    LOG_DBG("Destroying watcher");
    if (!watcher) {
        return;
    }
    sd_bus_unref(watcher->bus);
...
create_watcher(char *protocol, sd_bus *bus)
{
    ...
    ret = sd_bus_add_object_vtable(bus, NULL, watcher_path, watcher->interface, watcher_vtable, watcher);
    if (ret < 0) {
    ...
    watcher->bus = sd_bus_ref(bus);
    hosts_t tmp1 = tll_init();
    watcher->hosts = tmp1;
    items_t tmp2 = tll_init();
    watcher->items = tmp2;
    watcher->version = 0;
    LOG_DBG("Registered %s", watcher->interface);
    return watcher;
error:
...
handle_sni_registered(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    LOG_DBG("[Signal Handle] tray: handling registered '%s'", id);
...
{
    struct private *m = data;
    LOG_DBG("[Callback] tray: registering status notifier items");
    if (sd_bus_message_is_method_error(msg, NULL)) {
        const sd_bus_error *err = sd_bus_message_get_error(msg);
...
register_to_watcher(struct host *host)
{
    ...
    //
    int ret
        = sd_bus_call_method_async(host->m->bus, NULL, host->watcher_interface, watcher_path, host->watcher_interface,
    ...
    LOG_DBG("Registering watcher (%s) to host (%s)", host->watcher_interface, host->service);
    return true;
}
...
handle_new_watcher(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (!*old_owner) {
        if (strcmp(service, host->watcher_interface) == 0) {
            LOG_DBG("[Signal Handler] %s: registering service %s", host->service, service);
...
init_host(struct host *host, char *protocol, struct private *m)
{
    ...
    ret = sd_bus_match_signal(m->bus, NULL, host->watcher_interface, watcher_path, host->watcher_interface,
    ...
    ret = sd_bus_match_signal(m->bus, NULL, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus",
    ...
    LOG_DBG("[Init] Initializing Status Notifier Host: %s", host->service);
...
start_tray(struct private *m)
{
    ...
    if (!init_host(&m->host_xdg, "kde", m)) {
        LOG_ERR("Failed to start freedesktop host");
        goto err3;
    }
    if (!init_host(&m->host_kde, "freedesktop", m)) {
...
static void
destroy_tray(struct private *m)
{
    if (!m)
        return;

    // Destroy everything related to dbus since sd-bus isn't thread-safe.
    // Make sure not to touch anything that `content` could interactive with.
    finish_host(&m->host_xdg);
...
content(struct module *mod)
{
    ...
    tll_foreach(m->items, it)
    {
        ...
        LOG_DBG("%s status: %s", sni->watcher_id, sni->status);

        struct tag_set tags
            = {.tags = (struct tag *[]){
                   tag_new_string(mod, "watcher-id", sni->watcher_id), tag_new_string(mod, "title", sni->title),
                   tag_new_string(mod, "status", sni->status), tag_new_string(mod, "category", sni->category),
...
run(struct module *mod)
{
    ...
    mtx_unlock(&mod->lock);
    LOG_DBG("[Init] starting dbus loop");

    while (true) {
        struct pollfd fds[]
            = {{.fd = mod->abort_fd, .events = POLLIN}, {.fd = m->fd, .events = POLLIN | POLLHUP | POLLERR}};
